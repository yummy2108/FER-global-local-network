...
def train(fold, train_dataset,path, test_dataset,rerange):
    batch_size = 64
    ...
    for epoch in range(start_step, end_step+1):
        ...
        for index, data in enumerate(train_loader):
            ...
            out = out1 + out2
            fuck = loss_fun(out, label.reshape(-1))
            train_loss += float(fuck)
        ...
        if eval_acc > best_acc:
            best_acc = eval_acc
            best_epoch = epoch
            torch.save(net.state_dict(), path+'{}_best.pth'.format(rerange))
        """
        if (epoch+1) % 10 == 0:
            # 保存模型
            torch.save(net.state_dict(), path+'{}.pth'.format(epoch+1))
        """
    """
    x = range(0,151)
    y1 = train_loss_graph
    y2 = test_loss_graph
    plt.plot(x,y1,ls='-',lw=2, label='train_loss')
    plt.plot(x,y2,ls='-',lw=2, label='test_loss') 
    plt.legend()
...
for i in range(10):
    ...
    b_acc = 0
    for j in range(10):
        ep, acc = train(i,train_dataset,path,test_dataset,j)
        if acc > b_acc:
            b_acc = acc
            b_ep = ep

    test_ep.append(b_ep)
    test_res.append(b_acc)
end_time = time.time()
